  ...
  class Handler
    ...
    def update_fields(table_name,columns)
      ...
      if current_fields.nil?
        ...
        current_fields = MessagePack.unpack(current_fields['fields:'].sub(/timestamp=\d{1,}, value=/,''))
        updated_fields = current_fields + (columns - current_fields)
      end
      fields_table.put(table_name,'fields',MessagePack.pack(updated_fields)) unless current_fields == updated_fields
    end
    ...
    def show_fields(from, like, where)
      begin
        fields_table = $hbase.table('table_fields', $formatter)
        fields = MessagePack.unpack(fields_table.get(from)['fields:'].sub(/timestamp=\d{1,}, value=/,''))
